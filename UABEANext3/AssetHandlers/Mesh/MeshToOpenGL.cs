...
namespace UABEANext3.AssetHandlers.Mesh
{
    // like everyone else who came before me, this
    // code is based on UnityStudio/AssetStudio's Mesh.cs
    // https://github.com/Perfare/AssetStudio/blob/master/AssetStudio/Classes/Mesh.cs
    public class MeshToOpenGL
    {
        ...
        public MeshToOpenGL(Workspace workspace, AssetsFileInstance fileInst, AssetTypeValueField baseField)
        {
        ...
        public float[] ConvertFloatArray(byte[] data, int dims, VertexFormatV2 format)
        {
            var size = GetFormatSize(format);
            var count = data.Length / size;
